...
> AI 인터뷰 기반 라이프로그 서비스 **DORAN**의 프론트(React + Vite + TS) / 백엔드(Spring Boot) / AI 서버 간 협업용 통합 README입니다. 백엔드/AI 팀은 본 문서의 API 스펙과 DTO 예시를 기준으로 구현해 주세요.
...
```
[Login] → [Onboarding(최초 1회)] → [Main]
   → [Interview] (AI 질문 ↔ 사용자 답변 반복)
   → [Episode 저장]
   → [Episode 선택 → Generate Loading → Generate Complete]
   → [Library / BookDetail / PageViewer]
```
...
> 프론트는 AI 서버를 직접 호출하지 않습니다. **백엔드가 프록시**하여 호출하고 결과만 프론트에 반환합니다.
...
| `EntryRouter` | `isOnboarded()` localStorage 체크 | `GET /users/me`의 `onboarded` 사용 |
| `Interview.tsx` `createNewSession()` | 프론트 단독 생성 | `POST /interviews/start` |
| `Interview.tsx` mock 답변 | 하드코딩 `"그때가 참 따뜻했어요."` | STT 결과 → `POST /interviews/{id}/messages` |
| `Interview.tsx` mock 다음 질문 | `setTimeout` 1.2s 후 하드코딩 | `POST /interviews/{id}/next-question` 응답 사용 |
...
- [ ] 톤 규칙: `tone=warm`, `speechLevel=HONORIFIC`이면 존댓말, `noFabrication=true` 강제 (없는 사실 생성 금지)
